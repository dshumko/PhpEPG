...
//	mysql_query($sqlin,$connf);
	...
	$sqlu = 'UPDATE configas SET failas = 0 ';// Failo rashymo bitas
	mysql_query($sqlu,$connf);
//####################################################################################################
//#####################################################  Keichiam!!!##################################
$feit = fopen('/var/tv24_epg/now_02.eit', "r");
$fts = fopen('/var/tv24_epg/now_02.ts', 'w');	
//$cnt = 0;
$connfa = array($connf, $fts);
//fseek($feit, 2, SEEK_CUR);
//$data = fread($feit, 1);
while (ftell($feit) < filesize('/var/tv24_epg/now_02.eit')) {
//#####################################################################################################
$pirmdu = fread($feit, 2);
...
//	$sqlu2 = 'UPDATE configas SET failas = 1 ';// Failo rashymo bitas
//	mysql_query($sqlu2,$connf);
